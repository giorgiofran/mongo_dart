  ...
  group('op_msg_commands', () {
    ...
    group('Kill cursor Command:', () {
      test('test on existing cursor', () async {
        ...
        expect((result[keyCursorsKilled] as List).first, cursor.cursorId.value);
      ...
      test('test on non existing cursor', () async {
        var collectionName = getRandomCollectionName();
        var collection = db.collection(collectionName);
        var command = KillCursorsCommand(collection, [BsonLong(111111111111)]);
        var result = await command.execute();
        expect(result, isNotNull);
        expect(result[keyOk], 1.0);
